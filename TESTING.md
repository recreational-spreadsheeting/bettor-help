...
**First, give the CLI your DraftKings cookie.** A "cookie" here is the small piece of text
your browser uses to stay logged in to DraftKings. You copy it once and save it to a file:

1. In your web browser, log in to **[draftkings.com](https://www.draftkings.com)**.
2. Open your browser's **Developer Tools** (press **F12**, or right-click the page →
   **Inspect**), go to the **Network** tab, refresh the page, click any request to
   `draftkings.com`, and in the request headers find the **`Cookie:`** value. Copy the
   entire value (everything after `Cookie: `).
   > Not comfortable with developer tools? That's fine — ask Claude during the
   > `reconcile-contests` walkthrough and it will guide you through grabbing the cookie.
3. **Save it to the cookie file as a single line with no trailing newline:**

   **On Mac** — the file lives at `~/.config/bettor-help/dk_cookie_header.txt`. You can
   create it from Terminal (paste your cookie between the quotes):
   ```zsh
   mkdir -p ~/.config/bettor-help
   printf '%s' 'PASTE_YOUR_COOKIE_VALUE_HERE' > ~/.config/bettor-help/dk_cookie_header.txt
   ```

   **On Windows** — the file lives at
   `C:\Users\<YourUsername>\.config\bettor-help\dk_cookie_header.txt`. Create it from
   PowerShell (paste your cookie between the quotes):
   ```powershell
   New-Item -ItemType Directory -Force "$env:USERPROFILE\.config\bettor-help" | Out-Null
   Set-Content -NoNewline -Path "$env:USERPROFILE\.config\bettor-help\dk_cookie_header.txt" -Value 'PASTE_YOUR_COOKIE_VALUE_HERE'
   ```

   > **DK cookies expire** (you'll be re-logged-out periodically). If reconcile later says
   > your cookie is expired or DK redirected you to login, just repeat this step with a
   > fresh cookie value to overwrite the file.
...
Reconcile says your cookie is expired or that DK redirected to login? Grab a fresh cookie
and overwrite the file from Step 6c. Cookies expire regularly — this is normal.

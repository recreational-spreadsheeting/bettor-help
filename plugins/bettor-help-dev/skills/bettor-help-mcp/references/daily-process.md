...
- **`entries/`** — the DKEntries.csv you export from DK's Edit Entries page, renamed
  `DKEntries-<slate_type>-dg<id>.csv` so each draft group's export is distinct.
- **`swaps/`** — the near-lock Edit-Entries CSVs from `export_edit_entries`.
- **`results/`** — reconcile outputs (standings, computed P&L).
